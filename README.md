...
- 룩북 결과를 먼저 보여주고, 착용 아이템별 유사 상품 링크는 비동기로 제공
...
4. 쇼핑 링크 검색
   - 룩북 이미지와 설명을 먼저 표시한 뒤, 상품 링크는 카드 안에서 비동기로 업데이트합니다.
...
- 쇼핑 링크 웹검색은 최종 룩북 표시 이후 비동기로 실행합니다.
- `trend`, `plan`, `shopping` Responses API 호출에는 `OPENAI_SERVICE_TIER = "priority"`를 적용합니다.
- 이미지 2장은 병렬 생성합니다.
- 추가 속도 개선이 필요하면 쇼핑 링크 검색을 버튼형으로 바꾸거나, 핵심 아이템 3개만 검색하도록 줄일 수 있습니다.
