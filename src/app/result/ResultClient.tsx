...
export default function ResultClient() {
  ...
  async function runPipeline() {
    ...
    try {
      ...
      // Stays in "exiting" (slid up, off-screen) permanently - no reset to
      // "hidden" afterward, since that reset was itself a transform change
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/set-state-in-effect -- deliberate fetch-on-mount kickoff, not a render-cascade bug
    runPipeline();
  ...
  if (!keyword.trim()) {
    return (
      <div className="flex min-h-screen flex-col items-center justify-center gap-4 bg-zinc-50 px-6 text-center dark:bg-black">
        <p className="text-zinc-600 dark:text-zinc-400">키워드가 없어요. 홈에서 다시 시작해주세요.</p>
        <Link href="/" className="rounded-lg bg-black px-5 py-2 font-medium text-white dark:bg-white dark:text-black">
          홈으로
        </Link>
      </div>
    );
      ...
      <main className="mx-auto flex max-w-4xl flex-col gap-8 px-6 py-16">
        <div className="flex items-center justify-between">
          ...
          <p className="text-sm text-zinc-500">
            키워드: <span className="font-medium text-zinc-800 dark:text-zinc-200">{keyword}</span>
          </p>
        </div>
